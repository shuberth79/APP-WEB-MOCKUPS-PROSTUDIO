...
const UploadBox: React.FC<UploadBoxProps> = ({ image, onClick, label, icon }) => (
  <div
    onClick={onClick}
    className="border-2 border-dashed border-white/10 rounded-2xl p-6 text-center hover:bg-white/5 cursor-pointer transition-all bg-black/40 group shadow-inner"
  >
    {image ? (
...
const ChatSection: React.FC<ChatSectionProps> = ({
  ...
  <div className="bg-black/60 rounded-2xl p-4 border border-white/5 shadow-inner">
    ...
    <form onSubmit={onSend} className="flex gap-2">
      ...
      />
      <button type="submit" className={`bg-${color}-600/20 text-${color}-500 border border-${color}-600/30 px-4 rounded-xl text-[9px] font-black uppercase hover:bg-${color}-600 hover:text-white transition-all`}>OK</button>
    </form>
  </div>
);
  ...
  <div className="flex-1">
    ...
    <div className="relative group">
      <select
        ...
        className="w-full bg-white/5 border border-white/5 rounded-xl px-3 py-2.5 text-[10px] font-black focus:border-orange-500 outline-none appearance-none cursor-pointer hover:bg-white/10 transition-colors uppercase tracking-tight group-hover:border-white/20"
      >
...
const App: React.FC = () => {
  ...
  const [gallery, setGallery] = useState<GeneratedImage[]>(() => {
    const saved = localStorage.getItem('mockup_gallery');
  ...
  useEffect(() => {
    localStorage.setItem('mockup_gallery', JSON.stringify(gallery));
  }, [gallery]);
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-gray-200">
      {zoomImage && (
        ...
        >
          <div
            className="relative transition-transform duration-300 ease-out flex items-center justify-center overflow-auto max-w-full max-h-full"
            onClick={(e) => { e.stopPropagation(); cycleZoom(); }}
          >
            <img
              src={zoomImage}
              className={`rounded-lg shadow-2xl origin-center transition-transform duration-300`}
      ...
      <div className="pt-24 max-w-7xl mx-auto px-4 pb-20">
        {/* Tab Switcher - Reordered: GALLERY after GENERATE */}
        <div className="flex flex-wrap justify-center bg-[#111] p-1.5 rounded-2xl w-fit mx-auto mb-10 border border-white/5 shadow-2xl gap-1">
          {[
            ...
            <button
              ...
              className={`px-6 py-3 rounded-xl font-black text-[10px] transition-all tracking-widest uppercase ${activeTab === tab.id ? `bg-[#222] text-${tab.color}-500 shadow-xl border border-white/5` : 'text-gray-500 hover:text-gray-300'}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-4 space-y-6">
            ...
            {activeTab === 'generate' && (
              <div className="bg-[#111] p-6 rounded-[2rem] border border-white/5 space-y-6 shadow-2xl">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {MOCKUP_CATEGORIES.map(cat => (
                      <button
                        ...
                        className={`p-3 rounded-xl border flex flex-col items-center gap-1 transition-all ${selectedCategories.has(cat.id) ? 'bg-orange-600/10 border-orange-500 text-orange-500' : 'bg-white/5 border-transparent text-gray-500'}`}
                      >
                ...
                <div>
                  <label className="text-[10px] font-black text-orange-500 uppercase tracking-widest mb-3 block">2. Cantidad / Pack</label>
                  <div className="flex flex-wrap gap-2">
                    {MOCKUP_CATEGORIES.find(cat => selectedCategories.has(cat.id))?.allowedQuantities.map(q => (
                      <button
                        key={q}
                        onClick={() => setSelectedQuantity(q)}
                        className={`px-4 py-2 rounded-lg border text-[10px] font-black transition-all uppercase ${selectedQuantity === q ? 'bg-orange-500 text-white border-orange-500' : 'bg-white/5 border-white/10 text-gray-400'}`}
                      >
                        {q}
                      </button>
                    ))}
            ...
            {activeTab === 'gallery' && (
              <div className="bg-[#111] p-6 rounded-[2rem] border border-white/5 space-y-6 shadow-2xl">
                ...
                {gallery.length > 0 && (
                  ...
                  >
                    DESCARGAR SELECCIONADOS ({selectedInGallery.size})
                  </Button>
                )}
                ...
                <div className="grid grid-cols-2 gap-3 max-h-[500px] overflow-y-auto pr-2 scrollbar-hide">
                  {gallery.map(img => (
                    <div
                      ...
                      className={`group relative aspect-square bg-black/40 rounded-xl overflow-hidden border-2 cursor-pointer transition-all ${selectedInGallery.has(img.id) ? 'border-green-500 ring-2 ring-green-500/20 scale-95' : 'border-white/5 hover:border-white/20'}`}
                    >
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center p-2 gap-1 transition-opacity">
                        <button onClick={(e) => { e.stopPropagation(); setZoomImage(img.url); }} className="bg-white/10 hover:bg-white text-white hover:text-black w-full py-1.5 rounded text-[8px] font-black uppercase border border-white/10 transition-all">Zoom</button>
                        <button onClick={(e) => { e.stopPropagation(); setGallery(prev => prev.filter(g => g.id !== img.id)); }} className="bg-red-600 hover:bg-red-500 text-white w-full py-1.5 rounded text-[8px] font-black uppercase transition-all">Borrar</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-[#111] p-6 rounded-[2rem] border border-white/5 space-y-6 shadow-2xl">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button 
                        ...
                        className="bg-white/5 border border-white/10 p-4 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all"
                      >
                        📂 Importar
                      </button>
                      <button 
                        onClick={() => setShowBaseGalleryPicker(!showBaseGalleryPicker)}
                        className={`bg-white/5 border border-white/10 p-4 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all ${showBaseGalleryPicker ? 'border-pink-500 text-pink-500' : ''}`}
                      >
                        🖼️ Galería
                      </button>
                    </div>
                    ...
                    <div className="bg-black/40 border border-white/10 rounded-xl p-3 grid grid-cols-4 gap-2 max-h-40 overflow-y-auto scrollbar-hide">
                      {gallery.map(img => (
                        <img 
                          ...
                          className="aspect-square object-cover rounded cursor-pointer hover:ring-2 hover:ring-pink-500 transition-all"
                        />
                      ))}
                    </div>
                  )}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <SliderControl label="Mover X" min={0} max={100} value={manualMontageDesignX} onChange={setManualMontageDesignX} color="pink" unit="%" />
                    <SliderControl label="Mover Y" min={0} max={100} value={manualMontageDesignY} onChange={setManualMontageDesignY} color="pink" unit="%" />
                    <SliderControl label="Escala" min={1} max={35} value={manualMontageDesignScale} onChange={setManualMontageDesignScale} color="pink" unit="%" />
                    <SliderControl label="Rotación" min={-180} max={180} value={manualMontageDesignRotation} onChange={setManualMontageDesignRotation} color="pink" unit="°" />
                    <SliderControl label="Perspectiva X" min={-45} max={45} value={manualMontageDesignPerspectiveX} onChange={setManualMontageDesignPerspectiveX} color="pink" unit="°" />
                    <SliderControl label="Perspectiva Y" min={-45} max={45} value={manualMontageDesignPerspectiveY} onChange={setManualMontageDesignPerspectiveY} color="pink" unit="°" />
                    <SliderControl label="Transparencia" min={0} max={100} value={manualMontageDesignOpacity} onChange={setManualMontageDesignOpacity} color="pink" unit="%" />
                  </div>
            ...
            {activeTab === 'edit' && (
              <div className="bg-[#111] p-6 rounded-[2rem] border border-white/5 space-y-6 shadow-2xl">
                ...
                <div className="space-y-4">
                  ...
                  <input type="range" min="5" max="95" value={designSize} onChange={e => setDesignSize(parseInt(e.target.value))} className="w-full accent-blue-500" />
                </div>
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="bg-[#111] rounded-[3rem] border border-white/5 overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.7)] min-h-[650px] flex flex-col relative">
              <div className="bg-black/40 p-4 border-b border-white/5 flex flex-wrap items-center justify-between gap-4 px-8">
                ...
                <div className="flex gap-2">
                  {RESOLUTIONS.map(res => (
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-full text-[9px] font-black border transition-all ${resolution === res ? 'bg-white text-black border-white shadow-lg' : 'bg-white/5 text-gray-500 border-white/5 hover:border-white/10'}`}
                    >
                      {res}
                    </button>
                  ))}
              ...
              <div className="flex-1 flex items-center justify-center p-4 bg-black/20 relative group">
                {isProcessing && activeTab !== 'manualMontage' ? (
                  ...
                  />
                ) : (activeTab === 'gallery' ? gallery[0] : (activeTab === 'generate' ? history[0] : editingMockup)) ? (
                  <div className="relative cursor-zoom-in group/img" onClick={() => {
                    let img = activeTab === 'gallery' ? gallery[0]?.url : activeTab === 'generate' ? history[0]?.url : editingMockup?.url;
                    if (img) setZoomImage(img);
                  }}>
                    <img
                      src={(activeTab === 'gallery' ? gallery[0] : (activeTab === 'generate' ? history[0] : editingMockup))?.url}
                      className="max-w-full max-h-[75vh] rounded-[2rem] shadow-2xl border border-white/5 object-contain"
                      alt="Mockup"
                    />
                  </div>
                ) : (
